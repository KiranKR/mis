...
public class WorkOrderDao {
		...
		try {
			if(search.equals("") || criteria.equals("")){
				query = " SELECT pw.PWRK_ID,pw.PWRK_UID," + " t.TALUK_ID,"
						...
						+ " pw.PWRK_STAGE_IDEN " + " FROM program_works pw"
						...
						+ " where pw.PWRK_STAGE_IDEN >= 3 and pw.PWRK_ROW_STATUS = 0 " 
				...
				query = " SELECT pw.PWRK_ID,pw.PWRK_UID," + " t.TALUK_ID,"
						...
						+ " JOIN caste_category cc"
						+ " ON pw.PWRK_CSTCTG_ID = cc.CSTCTG_ID"
						+ " where pw.PWRK_STAGE_IDEN >= 3 and "+ condition +" and pw.PWRK_ROW_STATUS = 0 " 
						+ "and s.SECTN_SUBDIV_ID = "+UserUtil.getSessionValue(UserBeanConstants.LOGIN_ID_IN_SESSION)+" ";
			}
	...
	public PendingRep getDetail(String pwId) {
		...
		try {
			String query = " SELECT pw.PWRK_ID,pw.PWRK_UID," + " t.TALUK_ID,"
						 ...
						 + " pw.PWRK_ESTIMATE_NO," + " pw.PWRK_ESTIMATE_COST,"
						 + " pw.PWRK_ESTIMATE_DATE," + " pw.PWRK_INTIMATE_DATE, "
						 + " pw.PWRK_STAGE_IDEN,pw.PWRK_WO_NO,pw.PWRK_WO_ISSUE_DATE, pw.PWRK_REG_FEE, pw.PWRK_RTNO FROM program_works pw"
						 + " JOIN taluk t" + " ON pw.PWRK_TALUK_ID = t.TALUK_ID"
						 + " JOIN section s" + " ON pw.PWRK_SECTN_ID = s.SECTN_ID"
						 + " JOIN caste_category cc"
						 + " ON pw.PWRK_CSTCTG_ID = cc.CSTCTG_ID"
						 + " where pw.PWRK_STAGE_IDEN >= 3 and pw.PWRK_ID = " + pwId;
			connection = MysqlConnectionProvider.getNewConnection();
			preparedStatement = connection.prepareStatement(query);
			...
			while (rs.next()) {
				...
				rep.setRegstrFee(rs.getDouble(20));
				rep.setRegstrNo(rs.getString(21));
			...
			new MysqlConnectionProvider().releaseConnection(rs, null, preparedStatement, connection);
		}
		return rep;
	}
	
	public boolean orderUpdate(PendingRep rep){
		boolean updated = true;
		try {
			connection =  MysqlConnectionProvider.getNewConnection();
			String query = "update program_works SET "
							+"PWRK_STAGE_IDEN = 4, "
			   				+"PWRK_WO_NO = '"+rep.getWorkOrderNum()+"' "
			  				+",PWRK_WO_ISSUE_DATE = STR_TO_DATE('"+rep.getWrkOrderIssDt()+"','%d/%m/%Y') "
	...
	public List<DepositeBean> getStatusDateLst(){
		...
		try {
			...
			new MysqlConnectionProvider().releaseConnection(rs2, null, preparedStatement, connection);
		}
