...
public class WorkStatusDao {
		...
		try {
			if(search.equals("") || criteria.equals("")){
				query = " SELECT pw.PWRK_ID,pw.PWRK_UID," + " t.TALUK_ID,"
						...
						+ " pw.PWRK_STAGE_IDEN,pw.PWRK_WO_NO,pw.PWRK_WO_ISSUE_DATE  " + " FROM program_works pw"
						...
						+ " where pw.PWRK_STAGE_IDEN >= 5 and pw.PWRK_ROW_STATUS = 0 " 
				...
				query = " SELECT pw.PWRK_ID,pw.PWRK_UID," + " t.TALUK_ID,"
						...
						+ " JOIN caste_category cc"
						+ " ON pw.PWRK_CSTCTG_ID = cc.CSTCTG_ID"
						+ " where pw.PWRK_STAGE_IDEN >= 5 and "+ condition +" and pw.PWRK_ROW_STATUS = 0 " 
						+ "and s.SECTN_SUBDIV_ID = "+UserUtil.getSessionValue(UserBeanConstants.LOGIN_ID_IN_SESSION)+" ";
			}
	...
	public PendingRep getDetail(String pwId) {
		...
		try {
			String query = " SELECT pw.PWRK_ID,pw.PWRK_UID," + " t.TALUK_ID,"
						 ...
						 + " JOIN caste_category cc"
						 + " ON pw.PWRK_CSTCTG_ID = cc.CSTCTG_ID"
						 + " where pw.PWRK_STAGE_IDEN >= 5 and pw.PWRK_ID = " + pwId;
			connection = MysqlConnectionProvider.getNewConnection();
			preparedStatement = connection.prepareStatement(query);
	...
	public boolean statusUpdate(PendingRep rep){
		boolean updated = true;
		List<StatusDateBean> beans = null;
		beans = rep.getStatusDateBeans();
		String query = "";
		try {
			connection =  MysqlConnectionProvider.getNewConnection();
			connection.setAutoCommit(false);
			if(rep.getPwStageIden().equals("5")){
				
					query = "insert into program_status (PSTS_DATE,PSTS_PWRK_ID,PSTS_STS_ID,PSTS_STS_DATE,PSTS_STS_FLAG,PSTS_REMARKS,PSTS_REMARKS_BY,PSTS_ROW_STATUS ) VALUES (STR_TO_DATE('11/11/1111', '%d/%m/%Y'),"+rep.getPrgWrId()+",3,STR_TO_DATE('"+rep.getWrkUndrProgress()+"', '%d/%m/%Y'),0,' ',0,0)";
					preparedStatement = connection.prepareStatement(query);
					preparedStatement.execute();
					if(!rep.getWrkCompleted().equals("")){
						query = "insert into program_status (PSTS_DATE,PSTS_PWRK_ID,PSTS_STS_ID,PSTS_STS_DATE,PSTS_STS_FLAG,PSTS_REMARKS,PSTS_REMARKS_BY,PSTS_ROW_STATUS ) VALUES (STR_TO_DATE('11/11/1111', '%d/%m/%Y'),"+rep.getPrgWrId()+",4,STR_TO_DATE('"+rep.getWrkCompleted()+"', '%d/%m/%Y'),0,' ',0,0)";
						preparedStatement = connection.prepareStatement(query);
						preparedStatement.execute();
						query = "update program_works SET PWRK_STAGE_IDEN = 6 WHERE PWRK_ID = " + rep.getPrgWrId();
						preparedStatement = connection.prepareStatement(query);
						preparedStatement.execute();
					}else{
					query = "update program_works SET PWRK_STAGE_IDEN = 5 WHERE PWRK_ID = " + rep.getPrgWrId();
					preparedStatement = connection.prepareStatement(query);
					preparedStatement.execute();
					}
			}else if((rep.getPwStageIden().equals("4") && (!rep.getWrkCompleted().equals("")))){
			...
			}else if(rep.getPwStageIden().equals("5")){
				
				query = "update program_status SET PSTS_STS_DATE = STR_TO_DATE('"+rep.getWrkUndrProgress()+"', '%d/%m/%Y') WHERE PSTS_STS_ID = 3 and PSTS_PWRK_ID = "+rep.getPrgWrId();
				preparedStatement = connection.prepareStatement(query);
				preparedStatement.execute();
		
			}else if(rep.getPwStageIden().equals("6")){
					query = "update program_status SET PSTS_STS_DATE = STR_TO_DATE('"+rep.getWrkUndrProgress()+"', '%d/%m/%Y') WHERE PSTS_STS_ID = 3 and PSTS_PWRK_ID = "+rep.getPrgWrId();
					preparedStatement = connection.prepareStatement(query);
					preparedStatement.execute();
					
					query = "update program_status SET PSTS_STS_DATE = STR_TO_DATE('"+rep.getWrkCompleted()+"', '%d/%m/%Y') WHERE PSTS_STS_ID = 4 and PSTS_PWRK_ID = "+rep.getPrgWrId();
					preparedStatement = connection.prepareStatement(query);
					preparedStatement.execute();
			}
