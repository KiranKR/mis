...
public abstract class PageCodeBase {
	...
	static {
		...
		resource = ResourceBundle.getBundle("com.Util.messages_en", locale);
	}
	...
	protected static Object getBindingValue(String beanName) {
		FacesContext fc = FacesContext.getCurrentInstance();
		return fc.getApplication().createValueBinding(beanName).getValue(fc);
	}
	
	protected static void setBindingValue(String beanName,Object value) {
		FacesContext fc = FacesContext.getCurrentInstance();
		fc.getApplication().createValueBinding(beanName).setValue(fc,value);
	}
	
	
	
	//	 Used to build error and user messages	
	
	protected void addMessage(String errorCode){
		UDException ude = new UDException();
		ude.setErrorCode(errorCode);		
		FacesMessage fm = null;
		if(errorCode.endsWith("E"))
			fm = buildErrMsg(ude);
		else if(errorCode.endsWith("M"))
			fm = buildMsg(ude);
	...
	protected void addMessage(String errorCode, String refNo){
		UDException e = new UDException();
	...
	protected FacesMessage buildErrMsg(UDException ude) {
		String msg = "";
		if(resource.getString(ude.getErrorCode())!=null)
			msg = resource.getString(ude.getErrorCode());
		if(ude.getRefNo() !=null)
			msg += ude.getRefNo();
		FacesMessage fm = new FacesMessage();
		fm.setSeverity(FacesMessage.SEVERITY_ERROR);
		fm.setSummary(msg);
	...
	protected FacesMessage buildMsg(UDException ude) {
		String msg = "";
		if(resource.getString(ude.getErrorCode())!=null)
			msg = resource.getString(ude.getErrorCode());
		if(ude.getRefNo() !=null)
	...
	protected boolean nullCheck(Object object) {
		if (object == null || object.toString().length() == 0 || "".equals(object.toString()) || "0".equals(object.toString()) || "0.0".equals(object.toString()))
			return true;
		else
			return false;
	}
	...
	public boolean fromDateLessToDate(String frYr,String frMnt,String toYr,String toMnt){
		String frMntYr =frYr+AppUtil.getMnthId(frMnt);
	  	String toMntYr =toYr+AppUtil.getMnthId((toMnt));
	  	if(Integer.parseInt(frMntYr)>Integer.parseInt(toMntYr)){
	  		return true;
	  	}else {
	  		return false;
	  	}
